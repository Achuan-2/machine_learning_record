function Coordinate3D(scene,units){
 ...
 // 	var gridHelper_x_y = new THREE.GridHelper( 20, 20, 0x2C2C2C, 0x888888 );
	...
	const yLabel = new THREE.CSS2DObject( yDiv );
	...
	const zLabel = new THREE.CSS2DObject( zDiv );
...
function Coordinate2D(scene,units,camera=undefined) {
	...
	this.titleDiv.className = 'label';
	this.titleDiv.textContent = this.title;
	this.titleDiv.style.fontSize   = '2em';

	this.titleDiv.style.marginTop = '-10em';
	this.titleDiv.style.marginLeft = '0em';
	this.scene = scene
}
...
Coordinate2D.prototype.setTitle = function(name){
...
//x and y label
	...
	this.titleLabel = new THREE.CSS2DObject( this.titleDiv );
	this.titleLabel.position.set( 0, -20, 0 );

	
	this.arrowX.add( this.xLabel );
	
	this.arrowY.add( this.yLabel );
	this.gridHelper_x_y.add(this.titleLabel)
